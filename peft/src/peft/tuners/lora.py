...
class DiagonalLinear(nn.Module):
    def __init__(self, in_feature, out_feature, r=8):
        super(DiagonalLinear, self).__init__()
        self.r = r
        self.weight = nn.Parameter(torch.zeros(self.r, in_feature))
        self.weight_col = nn.Parameter(torch.zeros(out_feature, 1))
...
class Linear(nn.Linear, LoraLayer):
    ...
    def train(self, mode: bool = True):
        ...
        if not mode and self.merge_weights and not self.merged:
            r1 = 16
            if self.r > 0:
                self.weight.data += torch.repeat_interleave(self.lora_A.weight, self.weight.shape[0]//r1, dim=0) * self.lora_A.weight_col * self.weight.data * self.scaling
            self.merged = True
